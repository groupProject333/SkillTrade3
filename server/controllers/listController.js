...
module.exports = {
    ...
    db.Listing.create(req.body)
      .then(dbModel => {
        ...
        db.User.findOneAndUpdate(
          { username: dbModel.owner },
          { $push: { listing: dbModel._id } },
          { new: true }
        ).then(dbUser => {
          console.log(dbUser);
        });
  ...
  findByTags: function(req, res) {
    ...
    db.Listing.find({ hashtags: { $all: req.body.searchTags } }).then(function(
      dbListing
    ) {
      res.json(dbListing);
    });
  ...
  findOne: function(req, res) {
    ...
    db.Listing.findById({ _id: req.params.id }).then(dbListing => {
      res.json(dbListing);
    });
  }
